...
public:
    ...
    void Initialize(Node nod) {
        ...
        if (!file || !file_index) {
            ...
            file.close();
            file.open(file_name);
            file_index.open(file_index_name, std::fstream::out);
            file_index.close();
            file_index.open(file_index_name);
            NodeIndex head;
    ...
    void Insert(Node nod) {
        ...
        file.seekg(idx.block_begin, std::ios::beg);
        ...
        if (blo.now <= blo.maxn) {
            ...
            file.seekp(idx.block_begin, std::ios::beg);
            file.write(reinterpret_cast<char *>(&blo), sizeof(Block));
        } else {
            Block blo_new;
            NodeIndex idx_new;
            if (idx.next != -1) {
                ...
                if (!del.empty()) {
                    ...
                    file_index.seekg(0, std::ios::end);
    ...
    std::string FindMore(std::string index) {
        file.open(file_name);
        file_index.open(file_index_name);
        if (!file)
            return "";
        ...
        file.read(reinterpret_cast<char *>(&blo), sizeof(Block));
        std::string ans = "";
        ans += std::to_string(idx.block_begin) + " ";
        bool more = 0;
